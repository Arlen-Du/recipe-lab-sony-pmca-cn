...
public final class I18n {
    ...
    static {
        ...
        DICT.put("fav (hold)", "长按收藏"); // verified working on camera
        ...
        DICT.put("One JPEG per recipe, in table order — MENU stops the run", "自动连拍各配方, 按MENU停止");
        DICT.put("Wait after applying a recipe before the shutter fires", "应用配方后的快门等待时间");
        DICT.put("No live preview — the sample run needs the camera", "无实时预览 — 需要相机就绪");
    ...
    public static String tToast(String msg) {
        ...
        if (msg.startsWith("Samples done — ")) {
            return msg.replace("Samples done — ", "连拍完成 — ").replace("frames shot, listed in", "张已拍摄，记录于");
        }
        ...
        if (msg.startsWith("Sample run stopped — ")) {
            return msg.replace("Sample run stopped — ", "连拍停止 — ").replace("frames shot, listed in", "张已拍摄，记录于");
        }
